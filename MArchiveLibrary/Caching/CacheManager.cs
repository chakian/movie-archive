...
namespace MArchiveLibrary.Caching
{
    public class CacheManager
    {
        private static string GetCacheName(string cacheKey, string functionName)
        {
            return string.Format("{0}_{1}", cacheKey, functionName);
        }

        public static void WriteCache<T>(string cacheKey, string functionName, T cacheValue)
        {
            CacheItemPolicy policy = new CacheItemPolicy();
            string cacheName = GetCacheName(cacheKey, functionName);

            if (GetCache(cacheName) == null)
                InvalidateCache(cacheName);

            MemoryCache.Default.Set(cacheName, cacheValue, policy);
        }

        public static object GetCache(string cacheKey, string functionName)
        {
            object desiredCache = MemoryCache.Default.Get(GetCacheName(cacheKey, functionName));
        ...
        public static void InvalidateAllCacheByKey(string cacheKey)
        {
            var cacheListToRemove = MemoryCache.Default.Where(q => q.Key.StartsWith(GetCacheName(cacheKey, ""))).Select(q => q.Key).ToList();
            foreach (var item in cacheListToRemove)
            {
                MemoryCache.Default.Remove(item);
            }
        }
        public static void InvalidateCache(string cacheKey, string functionName)
        {
            InvalidateCache(GetCacheName(cacheKey, functionName));
        }
